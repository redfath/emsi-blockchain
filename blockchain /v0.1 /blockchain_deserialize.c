...
blockchain_t *blockchain_deserialize(char const *path)
{
	...
	if (read(fd, buf, strlen(HBLK_MAGIC)) != strlen(HBLK_MAGIC) || strcmp(buf, HBLK_MAGIC))
		return (CLEAN_UP, NULL);
		
	buf[strlen(HBLK_VERSION)] = 0;
	if (read(fd, buf, strlen(HBLK_VERSION)) != strlen(HBLK_VERSION) || strcmp(buf, HBLK_VERSION))
		return (CLEAN_UP, NULL);
